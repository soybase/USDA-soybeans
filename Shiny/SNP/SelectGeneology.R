...
getancestors <- function(bean, gen=0){
...
getdescendants <- function(bean, gen=0){
  if(is.na(bean)) return()
...
# 
# library(ggplot2)
# test <- plotcoords(node.to.data.frame(getancestors("Kent")))
...
# 
# test <- plotcoords(node.to.data.frame(getdescendants("Kent")))
# qplot(data=test, x=x, y=y, label=label, geom="text", vjust=0, hjust=.5) + 
#   geom_segment(aes(x=xstart, y=ystart, xend=xend, yend=yend)) + 
#   geom_segment(aes(x=xend, y=yend, xend=branchx, yend=branchy))
# 
...
# 
# 
# test <- plotcoords(rbind(node.to.data.frame(getancestors("Williams")), node.to.data.frame(getdescendants("Williams"))))
# qplot(data=test, x=x, y=y, label=label, geom="text", vjust=0, hjust=.5) + 
#   geom_segment(aes(x=xstart, y=ystart, xend=xend, yend=yend)) + 
#   geom_segment(aes(x=xend, y=yend, xend=branchx, yend=branchy)) +
#   theme_bw()
...
## Backup as of 1/1/14 - this plotcoords doesn't accommodate the amount of data present.
# # Calculates coordinates to plot each node on a tree (with corresponding edges). 
# # xlab, ylab describe the coordinates of the label
# # the line from (xend,  y) to (xstart, ystart) is the edge
# # the line from (xend, y) to (x, y) is the node (the horiz. line by the label)
# plotcoords <- function(df, sp=1){
# 
#   if(nrow(subset(df, root.gen==0 & gen==0))>1){
#     temp <- subset(df, root.gen==0 & gen==0)
#     temp$type <- "center"
#     temp <- unique(temp)
#     df <- rbind(subset(df, !(root.gen==0 & gen==0)), temp)
#   }
#   
#   # sort data frame
#   df <- df[order(df$type, df$root.gen, df$gen, df$par.id, df$branch),]
#   
#   # initialize x, y coords
#   gmax <- rep(max(abs(df$gen))+1, 2)
#   max.children <- rep(max((max(table(df$root)))^(2/3), 2)+1, 2)
#   fudge.factor <- c(1/3,1/3)
#   df$x <- 0
#   df$y <- 0
#   df$xstart <- 0
#   df$ystart <- 0
...
#   
# 
#   # Set subsequent generations based on previous generations
#   for(i in sort(unique(df$par.id), decr=F, na.last=FALSE)){
#     if(is.na(i)){ # if this is the root node and there are no parents
#       side <- (c(-1,1)[as.numeric(df$type=="descendant")+1])[which(is.na(df$par.id))]
#       df$x[which(is.na(df$par.id))] <- side*.5
#       df$xend[which(is.na(df$par.id))] <- df$x[which(is.na(df$par.id))] - side*(sp+.5)
#     } else { # child node
#       side <- (c(-1,1)[as.numeric(df$type=="descendant")+1])[which(df$par.id==i)]
#       da <- subset(df, par.id==i)
#       df$xstart[which(df$par.id==i)] <- side*pmax(abs(subset(df, id==i)$x), abs(subset(df, id==i)$xend))
#       df$ystart[which(df$par.id==i)] <- subset(df, id==i)$y
#       df$x[which(df$par.id==i)] <- side*pmax(abs(subset(df, id==i)$x), abs(subset(df, id==i)$xend)) + side*(max.children[(side+3)/2]^(gmax[(side+3)/2]-da$gen))^(fudge.factor[(side+3)/2])+side*(sp+.5)
#       df$y[which(df$par.id==i)] <- subset(df, id==i)$y + side*da$branch*(max.children[(side+3)/2]^(gmax[(side+3)/2]-da$gen))
#       df$xend[which(df$par.id==i)] <- df$x[which(df$par.id==i)]-side*(sp+.5)
#       df$ylab[which(df$par.id==i)] <- df$y[which(df$par.id==i)]+df$ylab[which(df$par.id==i)]
#     }
#   }
#   
#   df$xend[which(df$root.gen==0 & df$gen==0)] <- -sp
#   df$x[which(df$root.gen==0 & df$gen==0)] <- sp
#   
#   df$xlab <- (df$xend+df$x)/2
#   return(df)
# }