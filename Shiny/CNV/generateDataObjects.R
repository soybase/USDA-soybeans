...
yieldplot <- ggplot() + 
  geom_point(data=subset(parent.data,!is.na(Yield) & !is.na(Year)), aes(x=Year, y=Yield, showSelected=Cultivar), alpha=.75, size=5, colour="black", fill="white", chunk_vars=character()) +
  geom_point(data=yield.sum, aes(x=Year, y=Yield, clickSelects=Cultivar, colour=sequenced), size=3, guide="none", chunk_vars=character()) + 
  geom_point(data=yield.sum, aes(x=Year, y=Yield, showSelected=Cultivar, colour=sequenced), size=5, guide="none", chunk_vars=character()) + 
  make_text(yield.sum, 1965.5, 68, "Cultivar") +
  geom_text(data=subset(parent.data, parent!="Unknown" & !is.na(Yield)), aes(x=1965.5+x, y=24+y*6, label=sprintf("Parent - %s: %.2f", parent, Yield), showSelected=Cultivar), chunk_vars=character()) + 
...
proteinplot <- ggplot() + 
  geom_point(data=subset(parent.data,!is.na(Protein) & !is.na(Year)), 
             aes(x=Year, y=Protein, showSelected=Cultivar), 
             alpha=.75, size=5, colour="black", fill="white", 
             chunk_vars=character()) +
  geom_point(data=yield.sum, 
             aes(x=Year, y=Protein, clickSelects=Cultivar, colour=sequenced), 
             size=3, 
             chunk_vars=character()) +
  geom_point(data=yield.sum, 
             aes(x=Year, y=Protein, showSelected=Cultivar, colour=sequenced), 
             size=5, 
             chunk_vars=character()) + 
  scale_colour_manual("Sequenced", values=c("red", "blue")) + 
  make_text(yield.sum, 1965.5, 39, "Cultivar") + 
  geom_text(data=subset(parent.data, parent!="Unknown" & !is.na(Protein)), 
            aes(x=1965.5+x, y=30+y*1.5, 
                label=sprintf("Parent - %s: %.2f", parent, Protein),
                showSelected=Cultivar), 
            chunk_vars=character()) + 
  ggtitle("Protein") + 
...
oilplot <- ggplot() + 
  geom_point(data=subset(parent.data,!is.na(Oil) & !is.na(Year)), aes(x=Year, y=Oil, showSelected=Cultivar), alpha=.75, size=5, colour="black", fill="white", chunk_vars=character()) +
  geom_point(data=yield.sum, aes(x=Year, y=Oil, clickSelects=Cultivar, colour=sequenced), size=3, chunk_vars=character()) + 
  geom_point(data=yield.sum, aes(x=Year, y=Oil, showSelected=Cultivar, colour=sequenced), size=5, chunk_vars=character()) + 
  scale_colour_manual("Sequenced", values=c("red", "blue")) + 
  make_text(yield.sum, 1965.5, 21, "Cultivar") + 
  geom_text(data=subset(parent.data, parent!="Unknown" & !is.na(Oil)), aes(x=1965.5+x, y=16+y/1.5, label=sprintf("Parent - %s: %.2f", parent, Oil), showSelected=Cultivar), chunk_vars=character()) + 
  ggtitle("Oil") + 
  theme(legend.position="none") + 
  theme_animint(width=400, height=250)

proteinOil <- ggplot() + 
  geom_point(data=subset(parent.data,!is.na(Oil) & !is.na(Protein)), aes(x=Protein, y=Oil, showSelected=Cultivar), alpha=.75, size=5, colour="black", fill="white", chunk_vars=character()) +
  geom_point(data=yield.sum, aes(x=Protein, y=Oil, clickSelects=Cultivar, colour=sequenced), size=3, chunk_vars=character()) + 
  geom_point(data=yield.sum, aes(x=Protein, y=Oil, showSelected=Cultivar, colour=sequenced), size=5, chunk_vars=character()) + 
  scale_colour_manual("Sequenced", values=c("red", "blue")) + 
  make_text(yield.sum, 35, 21, "Cultivar") + 
  geom_text(data=parent.sub, aes(x=35+x, y=16.5+y/2, label=sprintf("Parent - %s: Protein = %.2f     Oil = %.2f", parent, Protein, Oil), showSelected=Cultivar), chunk_vars=character()) + 
  ggtitle("Protein and Oil Content") + 
  theme(legend.position="none") + 
  theme_animint(width=300, height=300)

yieldOil <- ggplot(data=yield.sum) + 
  geom_point(data=subset(parent.data,!is.na(Yield) & !is.na(Oil)), aes(x=Yield, y=Oil, showSelected=Cultivar), alpha=.75, size=5, colour="black", fill="white", chunk_vars=character()) +
  geom_point(data=yield.sum, aes(x=Yield, y=Oil, clickSelects=Cultivar, colour=sequenced), size=3, chunk_vars=character()) + 
  geom_point(data=yield.sum, aes(x=Yield, y=Oil, showSelected=Cultivar, colour=sequenced), size=5, chunk_vars=character()) + 
  scale_colour_manual("Sequenced", values=c("red", "blue")) + 
  make_text(yield.sum, 47, 21, "Cultivar") + 
  geom_text(data=parent.sub, aes(x=47+x, y=16.5+y/2, label=sprintf("Parent - %s: Yield = %.2f     Oil = %.2f", parent, Yield, Oil), showSelected=Cultivar), chunk_vars=character()) + 
  ggtitle("Yield and Oil Content") + 
  theme(legend.position="none") + 
  theme_animint(width=300, height=300)

yieldProtein <- ggplot() + 
  geom_point(data=subset(parent.data,!is.na(Yield) & !is.na(Protein)), aes(x=Yield, y=Protein, showSelected=Cultivar), alpha=.75, size=5, colour="black", fill="white", chunk_vars=character()) +
  geom_point(data=yield.sum, aes(x=Yield, y=Protein, clickSelects=Cultivar, colour=sequenced), size=3, chunk_vars=character()) + 
  geom_point(data=yield.sum, aes(x=Yield, y=Protein, showSelected=Cultivar, colour=sequenced), size=5, chunk_vars=character()) + 
  scale_colour_manual("Sequenced", values=c("red", "blue")) + 
  make_text(yield.sum, 47, 39, "Cultivar") + 
  geom_text(data=parent.sub, aes(x=47+x, y=30.5+y, label=sprintf("Parent - %s: Yield = %.2f     Protein = %.2f", parent, Yield, Protein), showSelected=Cultivar), chunk_vars=character()) + 
  ggtitle("Yield and Protein Content") + 
  theme(legend.position="none") + 
  theme_animint(width=300, height=300)

maturityplot <- ggplot() + 
  geom_point(data=subset(parent.data,!is.na(Maturity) & !is.na(Year)), aes(x=Year, y=Maturity, showSelected=Cultivar), alpha=.75, size=5, colour="black", fill="white", chunk_vars=character()) +
  geom_point(data=yield.sum, aes(x=Year, y=Maturity, clickSelects=Cultivar, colour=sequenced), size=3, chunk_vars=character()) + 
  geom_point(data=yield.sum, aes(x=Year, y=Maturity, showSelected=Cultivar, colour=sequenced), size=5, guide="none", chunk_vars=character()) + 
  make_text(yield.sum, 1965.5, 938, "Cultivar") +
  geom_text(data=subset(parent.data, parent!="Unknown" & !is.na(Maturity)), aes(x=1965.5+x, y=907+y*4, label=sprintf("Parent - %s: %.2f", parent, Yield), showSelected=Cultivar), chunk_vars=character()) + 
  scale_colour_manual("Sequenced", values=c("red", "blue")) + 
...
maturityplot <- ggplot() + 
  geom_point(data=subset(parent.data,!is.na(Maturity) & !is.na(Year)), aes(x=Year, y=Maturity, showSelected=Cultivar), alpha=.75, size=5, colour="black", fill="white", chunk_vars=character()) +
  geom_point(data=yield.sum, aes(x=Year, y=Maturity, clickSelects=Cultivar, colour=sequenced), size=3, guide="none", chunk_vars=character()) + 
  geom_point(data=yield.sum, aes(x=Year, y=Maturity, showSelected=Cultivar, colour=sequenced), size=5, guide="none", chunk_vars=character()) + 
  make_text(yield.sum, 1965.5, 938, "Cultivar") +
  geom_text(data=subset(parent.data, parent!="Unknown" & !is.na(Maturity)), aes(x=1965.5+x, y=907+y*4, label=sprintf("Parent - %s: %.2f", parent, Maturity), showSelected=Cultivar), chunk_vars=character()) + 
  scale_colour_manual("Sequenced", values=c("red", "blue")) + 
  scale_fill_manual("Sequenced", values=c("red", "blue")) + 
  ggtitle("Maturity")  + 
  theme_animint(width=400, height=250)
...
lodgingplot <- ggplot() + 
  geom_point(data=subset(parent.data,!is.na(Lodging) & !is.na(Year)), aes(x=Year, y=Lodging, showSelected=Cultivar), alpha=.75, size=5, colour="black", fill="white", chunk_vars=character()) +
  geom_point(data=yield.sum, aes(x=Year, y=Lodging, clickSelects=Cultivar, colour=sequenced), size=3, guide="none", chunk_vars=character()) + 
  geom_point(data=yield.sum, aes(x=Year, y=Lodging, showSelected=Cultivar, colour=sequenced), size=5, guide="none", chunk_vars=character()) + 
  make_text(yield.sum, 1965.5, 3.4, "Cultivar") +
  geom_text(data=subset(parent.data, parent!="Unknown" & !is.na(Lodging)), aes(x=1965.5+x, y=1+y/3, label=sprintf("Parent - %s: %.2f", parent, Lodging), showSelected=Cultivar), chunk_vars=character()) + 
  scale_colour_manual("Sequenced", values=c("red", "blue")) + 
  scale_fill_manual("Sequenced", values=c("red", "blue")) + 
  ggtitle("Lodging")  + 
  theme_animint(width=400, height=250)
...
seedsizeplot <- ggplot() + 
  geom_point(data=subset(parent.data,!is.na(SeedSize) & !is.na(Year)), aes(x=Year, y=SeedSize, showSelected=Cultivar), alpha=.75, size=5, colour="black", fill="white", chunk_vars=character()) +
  geom_point(data=yield.sum, aes(x=Year, y=SeedSize, clickSelects=Cultivar, colour=sequenced), size=3, guide="none", chunk_vars=character()) + 
  geom_point(data=yield.sum, aes(x=Year, y=SeedSize, showSelected=Cultivar, colour=sequenced), size=5, guide="none", chunk_vars=character()) + 
  make_text(yield.sum, 1965.5, 21, "Cultivar") +
  geom_text(data=subset(parent.data, parent!="Unknown" & !is.na(SeedSize)), aes(x=1965.5+x, y=10+y*1.5, label=sprintf("Parent - %s: %.2f", parent, SeedSize), showSelected=Cultivar), chunk_vars=character()) + 
  scale_colour_manual("Sequenced", values=c("red", "blue")) + 
  scale_fill_manual("Sequenced", values=c("red", "blue")) + 
  ggtitle("Seed Size")  + 
  theme_animint(width=400, height=250)
...
seedqualityplot <- ggplot() + 
  geom_point(data=subset(parent.data,!is.na(SeedQuality) & !is.na(Year)), aes(x=Year, y=SeedQuality, showSelected=Cultivar), alpha=.75, size=5, colour="black", fill="white", chunk_vars=character()) +
  geom_point(data=yield.sum, aes(x=Year, y=SeedQuality, clickSelects=Cultivar, colour=sequenced), size=3, guide="none", chunk_vars=character()) + 
  geom_point(data=yield.sum, aes(x=Year, y=SeedQuality, showSelected=Cultivar, colour=sequenced), size=5, guide="none", chunk_vars=character()) + 
  make_text(yield.sum, 1965.5, 3.3, "Cultivar") +
  geom_text(data=subset(parent.data, parent!="Unknown" & !is.na(SeedQuality)), aes(x=1965.5+x, y=1.2+y/3, label=sprintf("Parent - %s: %.2f", parent, SeedQuality), showSelected=Cultivar), chunk_vars=character()) + 
  scale_colour_manual("Sequenced", values=c("red", "blue")) + 
  scale_fill_manual("Sequenced", values=c("red", "blue")) + 
...
plcountplot <- ggplot() + 
  geom_point(data=subset(parent.data,!is.na(PlCount) & !is.na(Year)), aes(x=Year, y=PlCount, showSelected=Cultivar), alpha=.75, size=5, colour="black", fill="white", chunk_vars=character()) +
  geom_point(data=yield.sum, aes(x=Year, y=PlCount, clickSelects=Cultivar, colour=sequenced), size=3, chunk_vars=character()) + 
  geom_point(data=yield.sum, aes(x=Year, y=PlCount, showSelected=Cultivar, colour=sequenced), size=5, guide="none", chunk_vars=character()) + 
  make_text(yield.sum, 1965.5, 40, "Cultivar") +
  geom_text(data=subset(parent.data, parent!="Unknown" & !is.na(PlCount)), aes(x=1965.5+x, y=10+y*4, label=sprintf("Parent - %s: %.2f", parent, PlCount), showSelected=Cultivar), chunk_vars=character()) + 
  scale_colour_manual("Sequenced", values=c("red", "blue")) + 
  scale_fill_manual("Sequenced", values=c("red", "blue")) + 
  ggtitle("Pl Count")  + 
  theme_animint(width=400, height=250)
...
overview <- 
  ggplot() + 
  geom_tallrect(data=genome, aes(xmin=start, xmax=end, clickSelects=seqnames, fill=I("transparent")), alpha=.25, color="black", chunk_vars=character()) + 
  geom_area(data=overview.dens, aes(x=x, y=y, group=seqnames, clickSelects=seqnames, colour=factor(color), fill=factor(color)), alpha=2, inherit.aes=F, chunk_vars=character()) + 
...
varietyopts <- 
  ggplot() + 
  geom_tallrect(data=variety.opts, aes(xmin=rank-.45, xmax=rank+.45, clickSelects=Variety, showSelected=seqnames), alpha=.1, fill="grey80", chunk_vars=character()) + 
  geom_text(data=chr.summary, 
            aes(label=sprintf("Chromosome %s", gsub("Chr0?", "", seqnames)), 
                x=118/2, y=3000, showSelected=seqnames), chunk_vars=character()) + 
  geom_segment(data=variety.opts, aes(x=rank, xend=rank, y=CNVs, yend=0, tooltip=Variety, showSelected=seqnames), size=3, chunk_vars=character()) +
  geom_segment(data=variety.opts, aes(x=rank, xend=rank, y=CNVs, yend=0, color=factor(color), tooltip=Variety, showSelected=seqnames, showSelected2=Variety), size=3, chunk_vars=character()) + 
  geom_text(data=variety.opts, aes(x=rank, y=CNVs+offset*30, label=Variety, showSelected=seqnames, showSelected2=Variety, color=factor(color)), chunk_vars=character()) + 
...
chromosomeplot <- 
  ggplot() + 
  geom_line(data=chr.dens, aes(x=x, y=chr.y.scale, showSelected=seqnames, tooltip="Overall Density"), chunk_vars=character()) + 
  ...
  geom_text(data=chr.summary, 
            aes(label=sprintf("Chromosome %s", gsub("Chr0?", "", seqnames)), 
                x=mid, y=1, showSelected=seqnames), chunk_vars=character()) + 
  scale_colour_identity(guide="none") + 
...
varietydiffplot <- 
  ggplot() + 
  geom_hline(aes(yintercept=0), color="grey", chunk_vars=character()) + 
  geom_line(data=variety.density, aes(x=x, y=ydiff, group=Variety, colour=factor(color), tooltip=Variety, 
                                      clickSelects=Variety, showSelected2=seqnames, showSelected=Variety), chunk_vars=character()) + 
